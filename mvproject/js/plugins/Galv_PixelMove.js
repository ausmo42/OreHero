...
Game_Map.prototype.xWithDirectionQi = function(x, d) {
	return x + (d === 6 ? 0.33 : d === 4 ? -0.33 : 0);
};

Game_Map.prototype.yWithDirectionQi = function(y, d) {
    return y + (d === 2 ? 0.33 : d === 8 ? -0.33 : 0);
};

Game_Map.prototype.roundXWithDirectionQi = function(x, d) {
    return this.roundX(xWithDirectionQi(x,d));
};

Game_Map.prototype.roundYWithDirectionQi = function(y, d) {
    return this.roundY(yWithDirectionQi(y,d));
};
...
Game_Player.prototype.quadDirX = function(d) {
	...
	if (d === 6) {
		...
		quad = (this._tileQuadrant.x + 1) % 3;
	} else if (d === 4) {
		quad = (this._tileQuadrant.x + 3 - 1) % 3;
...
Game_Player.prototype.canPass = function(x, y, d, qx, qy) {
	...
	// 		break;
	// 	case 4: // Move Left
	// 		if (qx === 2) {//target left grid
	// 			if (!canPassTile) return false;
	// 			if (qy === 1) {//target top-left grid
	// 				var pass = Game_CharacterBase.prototype.canPass.call(this,x,y + 1,d);
	// 				if (!pass) this._diagSlide = Galv.PMOVE.getHorzVertDirs(7);
	// 				return pass;
	// 			}
	// 			if (qy === 2) {//target bottom-left grid
	// 				var pass = Game_CharacterBase.prototype.canPass.call(this,x,y - 1,d);
	...
	// 		break;
	// 	case 8: // Move Up
	// 		if (qy === 2) {//target bottom grid
	// 			if (!canPassTile) return false;
	// 			if (qx === 1) { //target bottom-right grid
	// 				var pass = Game_CharacterBase.prototype.canPass.call(this,x + 1,y,d);
	// 				if (!pass) this._diagSlide = Galv.PMOVE.getHorzVertDirs(7);
	// 				return pass;
	// 			}
	// 			if (qx === 2) { //target bottom-left grid
	// 				var pass = Game_CharacterBase.prototype.canPass.call(this,x - 1,y,d);
	// 				if (!pass) this._diagSlide = Galv.PMOVE.getHorzVertDirs(9);
	// 				return pass;
	// 			}
...
Game_Player.prototype.centerTilePos = function(face) {
	...
	this._tileQuadrant.x = 1;
	this._tileQuadrant.y = 1;
